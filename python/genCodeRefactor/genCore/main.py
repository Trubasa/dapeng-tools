...
# 核心应用流程
async def run_project_flow(project_name: str, step: int = 1):
    """
    应用程序的核心处理流程。
    :param project_name: 要处理的项目名称 (例如 'xy2miniprogram')
    :param step: 从哪一步开始执行，默认为1（第一步）
    """
    print(f'--- 欢迎使用代码生成器 ---')
    print(f'--- 正在处理项目: {project_name} ---')
    print(f'--- 从步骤 {step} 开始执行 ---')
    ...
    if step <= 1:
        ...
        # 如果跳过步骤1，但后续步骤需要配置，从保存的文件加载
        try:
            config_path = os.path.join(output_base_dir, 'config_output.json')
            if os.path.exists(config_path):
                with open(config_path, 'r', encoding='utf-8') as f:
                    app_config = json.load(f)
                print(f'从文件加载配置: {config_path}')
            else:
                print(f"错误: 未找到配置文件 {config_path}，无法继续执行。")
                return
    ...
    if step <= 2:
        ...
        # 如果跳过步骤2，但后续步骤需要聊天内容，从保存的文件加载
        try:
            chat_content_path = os.path.join(output_base_dir, 'chat_content_output.json')
            if os.path.exists(chat_content_path):
                with open(chat_content_path, 'r', encoding='utf-8') as f:
                    chat_content = json.load(f)
                content_preview = str(chat_content.get('content', ''))[:50].replace('\n', ' ')
                print(f'从文件加载聊天内容: {chat_content_path}')
                print(f"聊天内容 (预览): '{content_preview}...'")
            else:
                print(f"错误: 未找到聊天内容文件 {chat_content_path}，后续步骤可能无法正常执行。")
                chat_content = {"content": "", "imgDict": {}}
        except Exception as e:
            print(f"加载聊天内容文件时出错: {e}")
            chat_content = {"content": "", "imgDict": {}}
    ...
    if step <= 3:
        ...
        # 如果跳过步骤3，但后续步骤需要项目结构，尝试从文件加载
        project_structure_md_path = os.path.join(output_base_dir, 'project_structure.md')
        if os.path.exists(project_structure_md_path):
            try:
                with open(project_structure_md_path, 'r', encoding='utf-8') as f:
                    code_str_content = f.read()
                print(f"从文件加载项目结构: {project_structure_md_path}")
            except Exception as e:
                print(f"加载项目结构文件时出错: {e}")
                code_str_content = ""
        else:
            print(f"信息: 未找到项目结构文件 {project_structure_md_path}，API请求将缺少此信息。")
            code_str_content = ""
    ...
    if step <= 4:
        ...
        if not chat_query in chat_query:
        ...
        # 如果跳过步骤4，但步骤5需要知道API调用是否成功，检查输出文件是否存在
        extracted_output_path = os.path.join(output_base_dir, 'api_extracted_output.xml')
        if os.path.exists(extracted_output_path):
            print(f"检测到API输出文件: {extracted_output_path}")
            api_call_successful = True
        else:
            print(f"未检测到API输出文件: {extracted_output_path}，后续步骤可能无法执行。")
            api_call_successful = False
...
if __name__ == "__main__":
    ...
    parser.add_argument('-s', '--step', type=int, default=1, 
                        help='从哪一步开始执行 (1-5，默认为1)')
    args = parser.parse_args()
    
    # 验证步骤参数
    if args.step < 1 or args.step > 5:
        print(f"错误: 步骤参数必须在1到5之间，当前值: {args.step}", file=sys.stderr)
        sys.exit(1)
    
    try:
        asyncio.run(run_project_flow(args.project_name, args.step))
    except Exception as e:
